...
def get_neutron_client(conf) -> neutron_client.Client:
    """
    获取Neutron客户端
...
def list_external_networks(neutron_client: neutron_client.Client = None, **kwargs) -> List[Dict[str, Any]]:
    """
    获取所有的外部网络列表
    ...
    if neutron_client is None:
        neutron_client = get_neutron_client(**kwargs)
